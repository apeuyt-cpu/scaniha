...
export default function SocialMediaManager({ businessId }: SocialMediaManagerProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert('فشل الحفظ: ' + (err.message || 'حدث خطأ'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-5 lg:p-6" dir="rtl">
      <h2 className="text-xl lg:text-2xl font-bold text-zinc-900 mb-4">وسائل التواصل الاجتماعي</h2>
      <p className="text-sm lg:text-base text-zinc-600 mb-6">أضف روابط حساباتك على وسائل التواصل الاجتماعي</p>
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="mt-6 w-full px-6 py-3 bg-zinc-900 text-white rounded-xl font-semibold hover:bg-zinc-800 transition-colors disabled:opacity-50 text-sm lg:text-base"
      >
