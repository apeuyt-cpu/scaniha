...
export default function QuizQuestionsManager({ businessId }: { businessId: string }) {
  ...
  async function handleAdd() {
    ...
    const { error, data } = await sb.from('custom_game_questions').insert([insertData]).select().single()
  ...
  async function handleDelete() {
    ...
    const { error } = await sb.from('custom_game_questions').delete().eq('id', deletingId)
    if (error) {
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const { error } = await sb.from('custom_game_questions').insert(uniqueNewQuestions)
      ...
      {questions.length === 0 && !isAdding ? (
        ...
        <div className="grid gap-3">
          ...
          <ConfirmDialog
            ...
            description="Cette action est irréversible. La question ne sera plus posée dans le jeu."
