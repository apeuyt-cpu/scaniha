...
import { IconShare } from '@/components/admin/shell/icons'
...
/**
 * Re-homed "Partage & QR" section (was /admin/share): separate QR codes for the
 * menu and the fidelity program, the fidelity-QR landing picker, and the staff
 * access card. Logic unchanged from the legacy page.
 */
      ...
      <div className="flex flex-wrap gap-2">
        {LANDINGS.filter((l) => l.id !== 'roue' || hasRoulette).map((l) => (
          <button
            ...
            className={`rounded-xl px-3.5 py-2 text-sm font-semibold transition disabled:opacity-50 ${landing === l.id ? 'bg-[var(--brand)] text-white' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200'}`}
          >
            {l.label}
          </button>
        ))}
      </div>
      <p className="mt-2 text-[11px] text-[var(--muted)]">La première page que vos clients voient — par ex. la boutique des récompenses au lieu de la roue.</p>
    </div>
  )

  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <PageHeader title="Partage & QR" subtitle="Vos codes QR et liens à imprimer ou partager." icon={<IconShare width={20} height={20} />} />
      {mode === 'both' && (
        <p className="rounded-xl border border-[var(--line)] bg-zinc-50 px-4 py-3 text-sm text-zinc-600">
  ...
  return (
    <Card>
      ...
      <div className="mt-4 flex flex-col items-center gap-5 sm:flex-row sm:items-start">
        <div className="flex h-48 w-48 shrink-0 items-center justify-center rounded-2xl border border-[var(--line)] bg-white">
          {qr ? (
        ...
        <div className="w-full min-w-0 space-y-3">
          <div>
            ...
            <div className="flex items-center gap-2 rounded-xl border border-[var(--line)] bg-zinc-50 px-3 py-2">
              <code className="min-w-0 flex-1 truncate text-sm text-zinc-600" dir="ltr">{linkPath}</code>
              <button onClick={copy} disabled={!slug} className={`shrink-0 text-sm font-semibold disabled:opacity-50 ${copied ? 'text-green-600' : 'text-[var(--brand-600)] hover:text-[var(--brand)]'}`}>
                {copied ? 'Copié ✓' : 'Copier'}
              </button>
            </div>
            {gated && <p className="mt-1 text-[11px] text-[var(--muted)]">Le lien copié ne contient pas la clé de jeu — seul le QR imprimé débloque la roue.</p>}
          </div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <a href={plainUrl || '#'} target="_blank" rel="noopener noreferrer" className={`rounded-xl px-3 py-2.5 text-center text-sm font-semibold text-white transition ${plainUrl ? 'bg-[var(--brand)] hover:bg-[var(--brand-600)]' : 'pointer-events-none bg-zinc-300'}`}>
              {viewLabel}
            </a>
          </div>
        </div>
